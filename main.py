...
contr_polygon_address = w3.to_checksum_address('0x4803e859a2e325dc8f6adcd23ea682e323f59640')
...
for wallet_number, PRIVATE_KEY in numbered_keys:
    ...
    try:
        ...
        cprint("Вроде бы заебись", "green")
        time.sleep(random.randint(time_ot, time_do))
    except:
        cprint("Не заебись", "red")
        wallets_prob.append(address)

if len(wallets_prob) > 0:
    cprint("Есть проблемки", "red")
    print(wallets_prob)
else:
    cprint("Закончили без ошибок", "green")
